"use strict";(()=>{var ze=Object.defineProperty;var Me=(r,t)=>{for(var a in t)ze(r,a,{get:t[a],enumerable:!0})};var ee={};Me(ee,{Fragment:()=>K,ReactiveList:()=>D,call:()=>ce,cleanupNode:()=>S,createContext:()=>q,createElement:()=>je,createRoot:()=>Q,css:()=>n,cx:()=>N,registerCleanup:()=>W,useContext:()=>Z,useEffect:()=>M,useRef:()=>k,useState:()=>y});var ce=r=>typeof r=="function"&&"__register__"in r?r():r;var z={current:null},_=r=>r===null?"null":Array.isArray(r)?"array":r instanceof Date?"date":r instanceof Map?"map":r instanceof RegExp?"regexp":r instanceof Set?"set":typeof r,ie=(r,t)=>!Array.isArray(r)&&Object.prototype.hasOwnProperty.call(r,t);var D=class{constructor(t,a,c){this.subscribe=t;this.getItems=a;this.render=c}};var $=new WeakMap,W=(r,t)=>{let a=$.get(r);a?a.push(t):$.set(r,[t])},S=r=>{$.get(r)?.forEach(t=>t()),$.delete(r),Array.from(r.childNodes).forEach(S)},U=(r,t)=>{if(!(t==null||t===!1)){if(t instanceof Node){r.appendChild(t);return}if(t instanceof D){let a=document.createComment("");r.appendChild(a);let c=t,i=c.getItems().map(d=>{let m=c.render(d);return r.insertBefore(m,a),{item:d,node:m}}),l=c.subscribe(()=>{let d=c.getItems(),m=new Set,u=d.map(p=>{let h=i.findIndex((b,g)=>!m.has(g)&&b.item===p);return h>=0?(m.add(h),i[h]):{item:p,node:c.render(p)}});i.forEach((p,h)=>{m.has(h)||(S(p.node),p.node.parentNode?.removeChild(p.node))}),u.forEach(({node:p})=>a.parentNode?.insertBefore(p,a)),i=u});W(a,l);return}if(typeof t=="function"){let a=[],c=z.current;z.current=d=>{a.push(d)};let i;try{i=t()}finally{z.current=c}let l=i;if(typeof l=="function"&&l.__register__){let d=document.createTextNode(String(l())),m=l.__register__(()=>{d.textContent=String(l())});W(d,m),r.appendChild(d);return}if(a.length>0){let d=document.createComment("");r.appendChild(d);let m=g=>{let E=d.parentNode;if(g==null||typeof g=="boolean")return[];if(g instanceof DocumentFragment){let B=Array.from(g.childNodes);return B.forEach(T=>E.insertBefore(T,d)),B}let j=g instanceof Node?g:document.createTextNode(String(g));return E.insertBefore(j,d),[j]},u=g=>{g.forEach(E=>{S(E),E.parentNode?.removeChild(E)})},p=m(i),h=()=>{u(p),p=m(t())},b=a.map(g=>g(h));W(d,()=>b.forEach(g=>g()));return}U(r,i);return}r.appendChild(document.createTextNode(String(t)))}},K=(r,...t)=>{let a=document.createDocumentFragment();return t.forEach(c=>U(a,c)),a},je=(r,t,...a)=>{if(typeof r=="function")return r(t??{},...a);let c=document.createElement(r),i=t?.ref;return t&&Object.entries(t).forEach(([l,d])=>{l.startsWith("on")&&typeof d=="function"?c.addEventListener(l.slice(2).toLowerCase(),d):l==="className"?c.className=String(d):l==="htmlFor"?c.setAttribute("for",String(d)):typeof d=="boolean"?d&&c.setAttribute(l,""):l!=="ref"&&d!==null&&d!==void 0&&c.setAttribute(l,String(d))}),a.forEach(l=>U(c,l)),i&&(i.current=c),c};var Y=new WeakMap,Fe=r=>{let t=Y.get(r);return t||(t=[],Y.set(r,t)),t},q=r=>{let t={};return t.defaultValue=r,t.Provider=(a,...c)=>{let i=Fe(t);i.push(a.value);let l=K(null,...c);return i.pop(),l},t},Z=r=>{let t=Y.get(r);return t&&t.length>0?t[t.length-1]:r.defaultValue};var Q=r=>{if(!r)throw new Error("createRoot: container is null");return{render(t){S(r),r.innerHTML="",r.appendChild(t)}}};var le=new Set,I=null,Oe=()=>(I||(I=document.createElement("style"),I.setAttribute("data-dust-css",""),document.head.appendChild(I)),I),Be=r=>{let t=5381;for(let a=0;a<r.length;a++)t=(t<<5)+t^r.charCodeAt(a);return"d"+(t>>>0).toString(36)},n=(r,...t)=>{let a=String.raw({raw:r},...t).trim(),c=Be(a);return le.has(c)||(le.add(c),Oe().textContent+=`.${c}{${a}}
`),c};var N=(...r)=>r.filter(Boolean).join(" ");var _e=(r,t)=>{let a,c=()=>{a?.();let l=r();a=typeof l=="function"?l:void 0},i=t.map(l=>l?.__register__?.(c)).filter(l=>typeof l=="function");return c(),()=>{a?.(),i.forEach(l=>l())}},M=_e;var Ie=r=>({current:r}),k=Ie;var Ae=new Set(["push","pop","unshift","shift","splice","sort","reverse","fill","copyWithin"]),Ge=(r,t)=>{let a=_(r);if(a==="array"){let c=r,i=t,l=Math.max(c.length,i.length);for(let d=0;d<l;d++)d<i.length&&(c[d]=i[d]);return c.length=i.length,c}if(a==="object"){let c=r,i=t,l=new Set(Object.keys(c)),d=new Set(Object.keys(i));return d.forEach(m=>{let u=c[m],p=i[m],h=u?.__setter__;h?h(p):c[m]=p}),l.difference(d).forEach(m=>{c[m]=void 0}),c}return t},me=r=>{let t=r,a=[],c=m=>(a.push(m),()=>{let u=a.indexOf(m);u!==-1&&a.splice(u,1)}),i=m=>{let u=m;typeof m=="function"&&(u=m(t)),t=Ge(t,u),a.forEach(p=>p())},l=m=>{if(typeof m!="object"||m===null)return m;if(Array.isArray(m))return m.map(l);if(_(m)!=="object")return m;let{constructor:u}=m;if(u&&u!==Object)return m;let p=Object.assign({},m);return Object.entries(p).forEach(([h,b])=>{b?.__setter__&&(p[h]=b())}),p};return[new Proxy(()=>(z.current&&z.current(c),l(t)),{get(m,u,p){let h=_(t);switch(u){case"__register__":return c;case"__setter__":return i;case"toString":case"valueOf":return()=>l(t)}if(h==="array"&&typeof u=="string"&&Ae.has(u)){let b=t[u];return(...g)=>{let E=b.apply(t,g);return a.forEach(j=>j()),E}}if(h==="array"&&u==="map")return b=>new D(c,()=>t,b);if(h==="array"||h==="object"){let b=t[u];if(typeof b=="function")return b.__setter__?b:b.bind(t);if(ie(t,u)){let[g]=me(b);return t[u]=g,g}return b}return Reflect.get(m,u,p)},ownKeys(){return Object.keys(t)},getOwnPropertyDescriptor(m,u){let p=_(t);if((p==="array"||p==="object")&&t!==null&&typeof t=="object"&&u in t)return{enumerable:!0,configurable:!0}}}),i]},y=me;var A="",de=r=>{A=r.replace(/\/+$/,"")},X=()=>A,Le=()=>{window.addEventListener("popstate",()=>H(window.location)),document.addEventListener("click",a=>{let c=a.target;for(;c&&c.tagName!=="A";)c=c.parentElement;if(c){let i=c;if(i.origin!==window.location.origin||i.hash)return;a.preventDefault(),history.pushState({},"",i.href)}});let{pushState:r,replaceState:t}=history;history.pushState=(...a)=>{r.apply(history,a),H(a[2])},history.replaceState=(...a)=>{t.apply(history,a),H(a[2])}},ue=()=>document.createDocumentFragment(),G,pe=ue,fe={},ge=[],te=r=>{ge.push(r),G!==void 0&&r(G,pe)},he=r=>{Object.assign(fe,Object.fromEntries(Object.entries(r).map(([t,a])=>[be(t),a]))),G=void 0,H(window.location)},H=r=>{let t=be(r);if(G===t)return;G=t;let a={},c=ue;for(let[i,l]of Object.entries(fe)){let m=new RegExp(`^${i.replace(/:[^/]+/g,u=>`(?<${u.slice(1)}>[^/]+)`)}$`).exec(t);if(m){a=m.groups??{},c=l;break}}pe=c,We(a),ge.forEach(i=>i(t,c))},be=r=>{let t=String(r),a=(t.startsWith("/")?t:new URL(t).pathname).replace(/\/+$/,"");return A&&a.startsWith(A)&&(a=a.slice(A.length)||"/"),a||"/"},[xe,We]=y({});typeof window<"u"&&Le();var ye=new WeakMap,O=(r,...t)=>{let a=document.createDocumentFragment();return ye.set(a,{...r,children:t}),a},Ee=(r,t="/")=>r.reduce((a,c)=>{let i=ye.get(c);if(!i)return a;let l=`${t}${i.index?"":`/${i.path??""}`}`.replace(/\/+/g,"/").replace(/\/$/,"")||"/";if(i.component)return{...a,[l]:i.component};if(i.redirect){let d=i.redirect;return{...a,[l]:()=>(history.pushState({},"",`${X()}${d}`),document.createDocumentFragment())}}return{...a,...Ee(i.children,l)}},{}),$e=(r,...t)=>{let a=document.createElement("div");return te((c,i)=>{S(a),a.replaceChildren(i())}),he(Ee(t)),a},re=$e;var He={...ee},e=He;var Xe={base:n`
    position: sticky;
    top: 0;
    z-index: 100;
    display: flex;
